  ...
  static List<Evidence> extractEvidenceItems(String reportText) {
    ...
    try {
      ...
      for (final line in lines) {
        ...
        if (trimmedLine.contains(RegExp(r'Evidence\s+(ID|#|Number)', caseSensitive: false))) {
        ...
        // Description pattern
        else if (trimmedLine.contains(RegExp(r'Description:', caseSensitive: false))) {
          currentDescription = _extractValue(trimmedLine);
        }
        // Location pattern
        else if (trimmedLine.contains(RegExp(r'Location:', caseSensitive: false))) {
          currentLocation = _extractValue(trimmedLine);
        }
        // Confidence pattern
        else if (trimmedLine.contains(RegExp(r'Confidence:', caseSensitive: false))) {
          final confidenceStr = _extractValue(trimmedLine);
          currentConfidence = _parseConfidence(confidenceStr);
        }
        // Significance pattern
        else if (trimmedLine.contains(RegExp(r'(Significance|Forensic)', caseSensitive: false))) {
